PROGRAM tdse_pop_sdi
  ...
  REAL(dpk), ALLOCATABLE, DIMENSION(:)        :: pop_ll_e1, pop_ll_e2, bound_L      ! pop_e1e2(e1,e2)
  !
  REAL(dpk)                                   :: sum_this, sum_this_1, sum_this_2
  REAL(dpk)                                   :: pe, pop_icl
  COMPLEX(dpk)                                :: zsum, ground_sum
  !
  ...
  CALL read_w2e_ct(w2e,lmax,cycle_number)       ! read TDSE wavefunction $\Psi(t) = \sum_i c_{i}(t) \Phi_{i}$
  DO l = 0, lmax
...
!!!!!!!----------------------------------------------------------------------------------------------------------------------------
  ...
  ALLOCATE( bound_L(0:lmax) )
  bound_L = 0.0_dpk

  partial_in_L:DO  l = 0, lmax
  !Calculate the partial (l1,l2) contributions  to ground state population------------------------------
       zsum = 0
       ie = 1
       !write(*,*) "ground total population =", abs(w2e(l)%ct(ie))**2

       partial_population_of_ground:  DO ic = 1, w2e(l)%ncf  ! cfg index

         le1 = w2e(l)%l1(ic)
         le2 = w2e(l)%l2(ic)

         if ((le1 .eq. 0) .and. (le2 .eq. 0)) then

            zsum = abs(w2e(l)%cv(ie,ic) * w2e(l)%ct(ie))**2   !v_j(t) = S_i c_i(t)v_ij

            ground_sum = ground_sum + zsum                  !p_j(t) = |v_j(t)|^2

         end if

       ENDDO partial_population_of_ground

       !write(*,*) "partial population (0,0) =", ground_sum

!End calculation of partial (l1,l2) contributions  to ground state population-----------------------
!Calculate bound state population(and thus total single+double ionization yield)--------------------

    zsum = 0
    ie = 1
    ie_max = w2e(l)%net

      sum_all_2e_energies:  DO ie = 1, ie_max  !ie = 2e energy index

          if(w2e(l)%e2e(ie) < en_ion_1) then
            zsum = zsum + abs(w2e(l)%ct(ie))**2
          end if

      ENDDO sum_all_2e_energies

      bound_L(l) = zsum

      !End calculation of bound state population(and thus total single+double ionization yield)------------

  ENDDO partial_in_L

  write(*,*) "bound population for L=", 0, "=", bound_L(0)
  write(*,*) "bound population for L=", 1, "=", bound_L(1)
  write(*,*) "bound population for L=", 2, "=", bound_L(2)
  write(*,*) "bound population for L=", 3, "=", bound_L(3)
  write(*,*) "total bound population =", SUM(bound_L)
!!!!!!!----------------------------------------------------------------------------------------------------------------------------
!!!!!!!----------------------------------------------------------------------------------------------------------------------------
!!!!!!!----------------------------------------------------------------------------------------------------------------------------
  ...
  angular_symmetry_2e_zero_order: DO l = 0, lmax
     ...
     sum_all_configurations: DO ic = 1, w2e(l)%ncf  ! based on (L,ic) --->  e1,l1 and e2,l2
        ...
        !
        ! pop(L,e1,e2) = |S_i c_i(t) * v_ij|^2   (= pe)
        !
        ...
        pe      = w2e(l)%pvt_0(ic)
        pop_icl = rho_e1e(le1, k1) * rho_e1e(le2, k2) * pe


        ! ic <--> (k1 l1 ; k2 l2)
        !note in cfg-L.inp only the (k1 l1; k2 l2) appears and not the  (k2 l2; k1 l1)
        ...
        pick_di_channels: IF( (e1>0).AND.(e2>0) )  THEN
           ...
           !
           ! when l1 == l2  cfg-L.inp only k1 < k2 are included in the cfg-files
           ! so it is only P(k1,k2>k1) that is recorded here. But since l1=l2
           ! we recover for P(k1,k2) = P(k2,k1)
           !
           !
!           IF(le1 == le2) THEN
           IF( w2e(l)%chl_1(icl).EQ.w2e(l)%chl_2(icl)) THEN
              w2e(l)%popd_ee_Lll(icl,k2,k1) = pop_icl

              IF(k2.EQ.k1) w2e(l)%popd_ee_Lll(icl,k2,k1) = 2*pop_icl

           ENDIF

           ! when l1 != l2 only l1 < l2 are included in the cfg-files
           ! nevertheless ALL possible (k1,k2) appear in the cfg files
...
!   ENDDO
  ...
  DO icll = 1, n_ll
   ...
   data_file = "pes/yields_ll.dat"
   OPEN(nout,file=data_file)

   WRITE(nout,'(1a5,11X,2a20)') "nll"," DI(e1)","DI(e2)"
   WRITE(nout,'(3a5,1X,3a20)') "l1","l2","icll","pop_e1_ch", "pop_e2_ch", "pop_e1_ch/DI"
   WRITE(nout,'(a100)') "....................................................................................................."
   WRITE(nout,'(1I5,11X,2ES20.8)') n_ll, SUM(pop_ll_e1), SUM(pop_ll_e2)
   DO icll = 1, n_ll
      WRITE(nout,'(3I5,1X, 3ES20.8)') l1(icll), l2(icll), icll, pop_ll_e1(icll), pop_ll_e2(icll), pop_ll_e1(icll)/SUM(pop_ll_e1)
   ENDDO
   WRITE(nout,'(20X,2ES20.8)') SUM(pop_ll_e1), SUM(pop_ll_e1)
   WRITE(nout,'(a100)') "....................................................................................................."
   WRITE(nout,'(a40,a20)') "DI(e1)","DI(e2)"
   WRITE(nout,'(2a5,10X,3a20)') "l,","n_l", "popLe1","popLe2", "popLe1/DI(e1)"
   WRITE(nout,'(4a5,4a20)')      "l1","l2","chl","icl", "pop_Lll_e1","pop_Lll_e2","pop_Lll_e1/popLe1","pop_Lll_e1/DI(e1)"
   WRITE(nout,'(a100)') "....................................................................................................."
   WRITE(nout,'(20X,2ES20.8)') pop_di_1, pop_di_2
   DO l = 0, lmax
      WRITE(nout,'(2I5,10X,3ES20.8)') l, w2e(l)%chl_n,  w2e(l)%popd_L_1,   w2e(l)%popd_L_2, w2e(l)%popd_L_1/pop_di_1
      DO icl = 1, w2e(l)%chl_n
         l1( chl(l,icl) ) = w2e(l)%chl_1(icl)
         l2( chl(l,icl) ) = w2e(l)%chl_2(icl)

         WRITE(nout,'(4I5,4ES20.8)') l1(chl(l,icl)), l2(chl(l,icl)), chl(l,icl), icl, &
              &                      w2e(l)%popd_Lll_1(icl), w2e(l)%popd_Lll_2(icl),  &
              &                      w2e(l)%popd_Lll_1(icl)/w2e(l)%popd_L_1,          &
              &                      w2e(l)%popd_Lll_1(icl)/pop_di_1
      ENDDO
      WRITE(nout,'(a60)') "..................................................................."
   ENDDO
   CLOSE(nout)
...
!     PRINT*,
  ...
  ! electron '1'
  !
  ! Saves dP/dE1(l1,l2).  If l1 = l2 then the spectrum is multiplied by 2.
...
!        WRITE(nout,'(a1,1i5)') '&', n_cols
        DO k1 = 1, ne_1e_cut
           IF(e1e(le1,k1) > 0.0_dpk) THEN
              WRITE(nout,'(3E15.6)') e1e(le1,k1), n_equiv * w2e(l)%popd_e1_Lll(icl,k1)
           ENDIF
        ENDDO
        CLOSE(nout)
       ...
       partial_channels_l_2:DO icl = 1, w2e(l)%chl_n
          ...
          DO k2 = 1, ne_1e_cut
             IF(e1e(le2,k2) > 0.0_dpk) THEN
                WRITE(nout,'(3E15.6)') e1e(le2,k2), n_equiv * w2e(l)%popd_e2_Lll(icl,k2)
             ENDIF
          ENDDO
          CLOSE(nout)
...
!     WRITE(nout,'(a1,1i5)') '&', n_cols
     ...
     DO k1 = 1, ne_1e_cut
        IF(e1e(le1,k1) > 0.0_dpk) THEN
           WRITE(nout,'(3E15.6)') e1e(le1,k1), n_equiv * popl_e1(icll,k1)
        ENDIF
     ENDDO
...
!     WRITE(nout,'(a1,1i5)') '&', n_cols
     ...
     DO k2 = 1, ne_1e_cut
        IF(e1e(le2,k2) > 0.0_dpk) THEN
           WRITE(nout,'(3E15.6)') e1e(le2,k2), n_equiv * popl_e2(icll,k2)
        ENDIF
     ENDDO
  ...
  save_PES_2D_l1l2:DO icll = 1, n_ll
     ...
     WRITE(nout,'(i5)'    ) n_cols
     !
     save_e1e2_rho_half_1:DO k1 = 1, ne_1e_cut      !function values on the grid
        DO k2 = 1, ne_1e_cut
           IF( (e1e(le1,k1)>0).AND.(e1e(le2,k2)>0) )  THEN
        ...
        ENDDO
          WRITE(nout,'(3E16.8)')
     ENDDO save_e1e2_rho_half_1
     !the other half

     !save_e1e2_rho_half_2:DO k2 = 1, ne_1e_cut      !function values on the grid
      !  DO k1 = 1, ne_1e_cut
      !     IF( (e1e(le1,k1)>0).AND.(e1e(le2,k2)>0) )  THEN
      !        WRITE(nout,'(3E16.8)') e1e(le2,k2), e1e(le1,k1), popl_e1e2(icll,k1,k2)
      !     ENDIF
      !  ENDDO
      !  WRITE(nout,'(3E16.8)')
     !ENDDO save_e1e2_rho_half_2
