...
!Redundant variables now after reformulating loops. Fix this.
    ...
    integer :: i_n1, i_n2, i_n3, i_n4 !index for summing over one-electron energies
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!calcualte angular distribution
      !Testing clebschgordon coefficients and spherical harmonic routines.
      !  write(*,*) "clebsch l1, l2, m1, m2, L, M = ", cleb(2*2, 2*2, 2*2, 2*(-2), 2*4, 0)
      !  write(*,*) "sph-harm l, m = ", sphharm(1, -1, (pi/2), 0.0_dpk)
      !Working
       ...
       !allocate( work_A(1:8, 1:8, 1:4, 1:nof_points_angular, 1:nof_points_angular))
        ...
        write(*,*) "working total L = 0"


        !write(rd2efilename,"(a,i1,a,i1,a)") "ad2e/ad2e_l1l2_",l1_select, "_", l2_select, ".out"
        write(rd2efilename,"(a)") "ad2e/e0.15_L=2.out"
        open(19, file = rd2efilename, status = 'replace')

        !loop_n1: do i_n1 = 1, 60
          loop_n2: do i_n2 = 1, 60
              write(*,*) "calculating sum over n2. Currently on n2= ", i_n2

        !!!!!!CHOOSE n1 and n2 for the specific energy sharing!!!!!!!!!!!!!!!!!!!!!!
        !this example i_n1 = 10 chooses first electron ejection with the energy of the 10th energy state
        !    i_n1 = 10
        !    i_n2 = 20
        !must select closest n1,n2 to e1, e2 for various l1, l2 values due to discretization variance of energies


              work_A = 0.0_dpk

              loop_total_L: do l = 2, 2 !choose specific L value to calculate. Too lazy to delete L loop.

                !write(*,*) "Calculating total angular momentum:", l

                nof_l_ncf = w2e(l)%ncf !number of configuarations for L
                ie_max = w2e(l)%net    !number of 2e-states for L

                  loop_total_energy: do ie =  ndi_l(l), ie_max           !Loop over all DoubleIonizaed 2e-states
                  !write(*,*) "working", ie

                    pop_t_ie = w2e(l)%ct(ie)                            !TDSE coefficients

                      loop_l1l2: do ic = 1, nof_l_ncf                   !Sum over all configurations(for L)

                        nn1 = w2e(l)%n1(ic)                             !nn1 is energy index. Also indexes k values
                        nn2 = w2e(l)%n2(ic)                             !nn2 is energy index. Also indexes k values
                        ll1 = w2e(l)%l1(ic) !angular momentum l1 from configuration ic
                        ll2 = w2e(l)%l2(ic) !angular momentum l2 from configuration ic

                              !skip configuration cycle if k is negative(maybe REDUNDANT AS WE ARE CHOOSING SPECIFIC K VALUES)
                        if((k_value(ll1,nn1) .lt. 0) .or. (k_value(ll2,nn2) .lt. 0)) then
                          cycle loop_l1l2
                        end if

                        !IF statement selects the closest energies to the chosen e1,e2 energy sharing
                        !These are input manually the closest energy index from the en1e-l.out files
                        if (ll1 .eq. 0) then
                          i_n1 = 15
                        else if(ll1 .eq. 1) then
                          i_n1 = 14
                        else if(ll1 .eq. 2) then
                          i_n1 = 13
                        else if(ll1 .eq. 3) then
                          i_n1 = 12
                        else if(ll1 .eq. 4) then
                          i_n1 = 12
                        end if

                        !i_n3 is introduced for finding not only (n1=x,n2=all values) but also (n1=all values,n2=x)
                        if (ll2 .eq. 0) then
                          i_n3 = 15
                        else if(ll2 .eq. 1) then
                          i_n3 = 14
                        else if(ll2 .eq. 2) then
                          i_n3 = 13
                        else if(ll2 .eq. 3) then
                          i_n3 = 12
                        else if(ll2 .eq. 4) then
                          i_n3 = 12
                        end if
                        !i_n3 is introduced for finding not only (n1=x,n2=all values) but also (n1=all values,n2=x)


                        !IF statement will discard cofnigurations not relating to chosen 1-e energies i_n1, i_n2
                        !Example: finding not only (n1=x,n2=all values) but also (n1=all values,n2=x)
                        if (((nn1 .eq. i_n1) .and. (nn2 .eq. i_n2)) .or. ((nn2 .eq. i_n3) .and. (nn1 .eq. i_n2))) then
                          ...
                          !write(*,*) nn1, ll1, ll2, nn2, pop_ic, abs(pop_t_ie)**2

                          !begin looping equation
                          !second if statement adds factor of 2 required for
                          theta1r = 0
                          theta2r = 0
                          angle_step = 2*pi/nof_points_angular

                          if (((nn1 .eq. nn2) .and. (ll1 .eq. ll2))) then
                            loop_angle: do theta2 = 1, nof_points_angular
                            ...
                            loop_angle_exchange: do theta2 = 1, nof_points_angular
                              work_A(theta2) =  work_A(theta2) + &
                  ...
                  if (i_n1 .eq. i_n2) then
                    A = A + abs(work_A)**2    !calcualte |c(k1,k2)|^2
                  else
                    !A = A + 2*abs(work_A)**2  !calcualte |c(k1,k2)|^2  (actually this factor of two here is only needed for total distribution)
                    !in order to save time by not calculating full n1,n2 matrix. For specific n1,n2 this will give the true distribution x2.
                    A = A + abs(work_A)**2
                  end if
