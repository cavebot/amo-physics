...
CONTAINS
  ...
  SUBROUTINE READ_FIELD
    ...
    ELSE IF(n_modes < 0) THEN
       ...
       omega = w_i((n_modes-1)/2) ! should be    = (w_max + w_min)/2.0_dpk

       make_amplitude_components:DO i = 1, n_modes
          e_i(i) = EXP( - ( (w_i(i) - omega)/dw_fwhm)**2)
...
!
  ...
  SUBROUTINE SAVE_FIELD(ti, nout_T)
    ...
    WRITE( nout,  '(a30,2E25.10)') '&                  w = ', omega,omega*enau  ! a.u., SI
...
!    td = 20.
!    DO iout = -cycles*nout_T, cycles*nout_T

      IF ( PULSETYPE.EQ.'acos2tail' ) THEN
        cycles = cycles + 4
        write(*,*) 'Added zero-tail to pulse (i.e 12 cycle pulse + 4 cycle of zero electric field)'
      END IF



    DO iout = 1, cycles*nout_T
...
!      CHARACTER(DPK)        PULSETYPE
  ...
  REAL(DPK) FUNCTION A_T(T)
      ...
      gauss_pulse:IF ( PULSETYPE.EQ.'agauss' ) THEN
         ...
         A_T = E0 * SHAPE * DCOS( omega * T + PHI )/omega
      ...
      cos2_pulse:IF ( PULSETYPE.EQ.'acos2' ) THEN
         ...
         A_T = E0 * SHAPE * SIN( omega * T + PHI )/omega

      ENDIF cos2_pulse

      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      cos2tail_pulse:IF ( PULSETYPE.EQ.'acos2tail' ) THEN
         IF ( DABS( 2.D0 * T ).LT.TAU ) THEN
            SHAPE = DCOS( M_PI * T /TAU ) ** 2
          ELSE
            SHAPE = 0
 ...
 !     stop
      ...
      cos2flat_pulse:IF (PULSETYPE.EQ.'acos2flat') THEN
         ...
         T1 = DINT( TAU ) / 1.0e+02_dpk
         T2 = ( TAU - DINT( TAU ) ) * 1.0e+4_dpk
         T2 = 2.0_dpk * T2
         SHAPE = 0.0_dpk

         IF ( DABS( T ).LT.( T1 + T2/2.0_dpk ) ) THEN
      ...
      IF (PULSETYPE.EQ.'esin2') THEN
         ...
         A_T = 0.5_dpk * E0 *  ( SHAPE * DCOS( omega * T + PHI ) &
              &         +      SHAPE_DER * DSIN( omega * T + PHI )  )/omega
  ...
  REAL(DPK) FUNCTION E_T(T)
    ...
    !
      ...
      gauss_pulse:IF ( PULSETYPE.EQ.'agauss' ) THEN
         ...
         SHAPE = DEXP( - 0.5_dpk * ( T / TAU ) * ( T / TAU ) )
...
!
      ...
      IF (PULSETYPE.EQ.'esin2') THEN
         ...
         E_T = E0 * SHAPE * DCOS( omega * T + PHI )

      ELSE

         E_T = E0 * (                SHAPE * DSIN( omega * T + PHI )    &
                     - SHAPE_DERIV * DCOS( omega * T + PHI )/omega )
