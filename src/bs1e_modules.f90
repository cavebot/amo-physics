  ...
  REAL(dpk) FUNCTION v1e(x, l)
    ...
    SELECT CASE(potential)
       ...
       case('model')                 !various atoms
          v1e = v_model(x,l)
       CASE('qdot')                 !3D spherical dot  -V0 * exp(-beta*r^2)
          v1e = v_qdot(x)
       case('well')                 !3D spherica well
          v1e = v_well(x)
       END SELECT
...
!..............................................
  ...
  REAL(dpk) FUNCTION v_well(x)
    ...
    width = SQRT(LOG(2.0)/znuc)

    v_well = 0.0_dpk
    IF(x.LE.width)      v_well = - ap
...
!!%//                                        r_(i,p)
!!%//
!!%//          i   :  t_i points on the interval [0, R]        ( n-k ) points
!!%//          p   :  Gaussian points inside the interval [t_i, t_i+1]   ( k  )  points
!!%//          j   :  number of B-spline     : B^(m)_j(r_{ip})
!!%//          m   :  derivative of B-splines
!!%//
  !
  !
  !              B_j^(m)(r_{ip})   : value of the 'm-th' derivative of the j-Bspline at the point r_{ip}
  !                                : where 'i'   defines the interval [t_i,t_{i+1}]
  !                                : and   'p-th'-Gaussian point
  !
     ...
     loop_over_non_vanishing_matrix_elements:DO  j = i, jhi          ! < B_i | O |B_{i+k} >
        ...
        loop_over_segments:DO  l = low, lhi           !sum over intermediate segments
           ...
           DO  m = 1, kb                      !sum over gaussian weights
              ...
              CALL dfspvd(t, kb, r, l, db, nderiv)

              ! NOTE: ALTERNATIVE THE BELOW ROUTINE CAN ALSO BE USED
              !       AS IS DONE IN THE C++ VERSION (BspBasis.C)
              !CALL dbspvd(t, kb, nderiv, r, l, kb, db, work)
