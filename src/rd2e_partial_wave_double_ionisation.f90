              ...
              read(16,'(3E20.10)') r(1) , p(1, i, l+1)
                ...
                loop_wavefunction_grid: do ir = 2, 2000
                   ...
                   if (mod(ir, mod_dummy) .eq. 0) then
                      ...
                      if((i .eq. 1) .and. (l.eq.0)) then
                        !write(*,*) r(ir/mod_dummy) , p((ir/mod_dummy), i, l+1)
                      end if
...
!Find ionisation thresholds
     ...
     ALLOCATE( nsi_l(0:lmax) )
       ...
       write(*,*) "#dummie 1 = ", dummie1
       write(*,*) "#dummie 2 = ", dummie2
       write(*,*) "#dummie 3 = ", dummie3
       write(*,*) "#dummie 4 = ", dummie4
       write(*,*) "#dummie 5 = ", dummie5
       write(*,*) "#Number of b-splines = ", ijmax
       write(*,*) "#dummie 6 = ", dummie6

write(rd2efilename,"(a,i1,i1,a)") "rd2e/partial-waves/time-evolution/f-di-pw-", l1_int,l2_int, "-cycle-"
...
omegaEV = 27.211 * omega

write(*,*)
WRITE(*, "(A22, F4.2, A9, F5.2, A3)") '# Photon energy Wph = ', omega, 'a.u.  or ', omegaEV, ' eV'
             ...
             do ir2 = 1, nof_points2
               if ((ir2 .ge. ir1)) then
                 if (f(ir1,ir2) .lt. 0) then
                   write(19,'(3E20.10)')  r(ir1)*0.0529, r(ir2)*0.0529, 0.00000 !SCALED.ATOMIC.UNITS TO nm
                 else
                   write(19,'(3E20.10)')  r(ir1)*0.0529, r(ir2)*0.0529, f(ir1,ir2) !SCALED.ATOMIC.UNITS TO nm
                 end if
               else
                 if (f(ir2,ir1) .lt. 0) then
                   write(19,'(3E20.10)')  r(ir1)*0.0529, r(ir2)*0.0529, 0.00000 !SCALED.ATOMIC.UNITS TO nm
                 else
                   write(19,'(3E20.10)')  r(ir1)*0.0529, r(ir2)*0.0529, f(ir2,ir1) !SCALED.ATOMIC.UNITS TO nm
                 end if
               end if
             end do
             write(19,'(3E20.10)')
