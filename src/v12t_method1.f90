...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Calculate SI, DI thresholds
  ...
  !Allocate array to store energy matrix
  ALLOCATE( enlnl(ie_max,ie_max) )

  !open  E_NLN'L matrix file
  WRITE(enlnl_outfile, "(a,i1,a)") "dat/e0_matrix-", l_int, ".dat"
  OPEN(nout, file = enlnl_outfile, form = 'unformatted')
  Write(*,*) "Opened zero-order energy matrix file"
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  DO ie = 1, ie_max   !SUM OVER N
   ...
   write(*,*) "<H(t)> value for L =", l_int, "=", ht*27.2113961, "eV"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !Calculate ZERO-ORDER HAMILTONIAN Expectation value
  h_0t = 0 !H(t) initialize

  DO ie = 1, ie_max   !SUM OVER N

    ct1 = w2e(l_int)%ct(ie)

     DO iep = 1, ie_max  !SUM OVER N', upper triangle

       ct2 = w2e(l_int)%ct(iep)
       h_0t = h_0t +  CONJG(ct1) * ct2 * enlnl(ie,iep)

     END DO
  END DO

  write(*,*) "<H_0(t)> value for L =", l_int, "=", h_0t*27.2113961, "eV"
  write(*,*) "<V(t)> value for L =", l_int, "=", (ht-h_0t)*27.2113961, "eV"

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
WRITE(Hout_outfile, "(a,i1,a)") "v12t/method1/H_out_", l_int, ".out"
OPEN(3, file = Hout_outfile, access = 'append')

write(3,'(i2,3E20.10)') num_cycles_int, ht*27.2113961, h_0t*27.2113961, (ht-h_0t)*27.2113961
close(3)
