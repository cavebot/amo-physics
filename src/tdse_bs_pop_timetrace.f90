PROGRAM tdse_pop_sdi
  ...
  REAL(dpk), ALLOCATABLE, DIMENSION(:,:)      :: pop_e1e2, work        ! pop_e1e2(e1,e2)
  ...
  CALL getarg(1,  num_cycles)
  READ(num_cycles(1:2),  "(i2)")  num_cycles_int
  write(*,*) num_cycles_int
  ...
  CALL read_w2e_ct(w2e,lmax,num_cycles_int)       ! read TDSE wavefunction $\Psi(t) = \sum_i c_{i}(t) \Phi_{i}$
  DO l = 0, lmax
...
!     PRINT*,
  ...
  save_PES_1D1_Ll1l2: DO l = 0, lmax
     partial_channels_l:DO icl = 1, w2e(l)%chl_n
        ...
        data_file = TRIM(pop_file)//"1D1_LC"//TRIM(ADJUSTL(sl))//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
        OPEN(nout,file=data_file, status = 'replace')
...
!        WRITE(nout,'(a1,1i5)') '&', n_cols
  ...
  n_equiv = 1
    ...
    save_PES_1D2_Ll1l2: DO l = 0, lmax
       partial_channels_l_2:DO icl = 1, w2e(l)%chl_n
          ...
          data_file = TRIM(pop_file)//"1D2_LC"//TRIM(ADJUSTL(sl))//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
          OPEN(nout,file=data_file, status = 'replace')
  ...
  partial_channels_1:DO icll = 1, n_ll
     ...
     data_file = TRIM(pop_file)//"1D1_C"//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
     OPEN(nout,file=data_file, status = 'replace')
...
!     WRITE(nout,'(a1,1i5)') '&', n_cols
  ...
  partial_channels_2:DO icll = 1, n_ll
     ...
     data_file = TRIM(pop_file)//"1D2_C"//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
     OPEN(nout,file=data_file, status = 'replace')
...
!     WRITE(nout,'(a1,1i5)') '&', n_cols
  ...
  save_PES_2D_Ll1l2: DO l = 0, lmax
     partial_channelsL:DO icl = 1, w2e(l)%chl_n
        ...
        data_file = TRIM(pop_file)//"2D_LC"//TRIM(ADJUSTL(sl))//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
        OPEN(nout,file=data_file, status = 'replace')
        ...
        n_cols = 3
        WRITE(nout,'(a1,4a5)') '&',"icl","l","l1", "l2"
        WRITE(nout,'(a1,5I5)') '&',  icl, l, le1, le2, num_cycles_int
  ...
  save_PES_2D_l1l2:DO icll = 1, n_ll
     ...
     WRITE(sl1, '(I6)') icll
     !
     data_file = TRIM(pop_file)//"2D_C"//TRIM(ADJUSTL(sl1))//"_"//TRIM(ADJUSTL(num_cycles))//".dat"
     OPEN(nout,file=data_file, status = 'replace')
     !
     le1 = l1(icll)
     le2 = l2(icll)
  ...
  data_file = "pes/pop_bound.dat"
  OPEN(200,file=data_file, access='append')
  WRITE(200,*) num_cycles_int,  popl_b_v,  SUM(popl_b_v)
  close(200)

  data_file = "pes/pop_single.dat"
  OPEN(201,file=data_file, access='append')
  WRITE(201,*) num_cycles_int, popl_s_v,  SUM(popl_s_v)
  close(201)

  data_file = "pes/pop_double.dat"
  OPEN(201,file=data_file, access='append')
  WRITE(201,*) num_cycles_int, popl_d_v,  SUM(popl_d_v)
  close(201)
