...
!xxxxxxxxxxx                    hohg        xxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  WRITE(*,*)'#             photon energy        Wph = ', omega ,    ' a.u.(s.a.u. for Q-Dot) ', omega*27.211 , 'eV(or s.eV for Q-Dot)'
  ...
  WRITE(*,*)'#             nof cycles       cycles  = ', cycles,    ' cycles'
  WRITE(*,*)'#'
  WRITE(*,*)'#                                   atom:'
  WRITE(*,*)'#'
  ...
  write(*,*) "opening correct dipole expectation value file for harmonic generation calculations"

  IF ( PULSETYPE.EQ.'acos2tail' ) THEN
    write(*,*) 'Number of cycles = ', cycles-4, 'plus 4 cycle of zero electric field '
  END IF
...
210  lstart = INT(0.5_dpk * ( t + 0.5_dpk * tau ) /Tp ) + 1
  ...
  IF(hohg) THEN
     tSteps = 128
     ALLOCATE(     time(nfft)  )
     ALLOCATE(     rt(nfft,lmax+1) )
     ALLOCATE(     at(nfft,lmax+1) )
     ALLOCATE(     vt(nfft,lmax+1) )
  ENDIF
...
!  ALLOCATE(work(100+21*neq))         ! work array for ode
  ...
  time_loop:DO  l = lstart, cycles !* tSteps
     ...
     time_loop_for_hhg: DO  lk = 1, tSteps
        ...
        ifail = 0
        !write(*,*) t-ti, A_t(time(kt)), int(((t-ti)/(tp*cycles))*100),"% completion"
        write(*,*) A_t(t)
        IF(gauge.EQ.'v') THEN
        ...
        harmonics:IF(hohg) THEN
           ...
           !write(*,*) "dzr 1,1  1,2   2,1 = ", dzr(1,1,1), dzr(2,1,1), dzr(1,2,1)
...
!
           ...
           loop_vt:DO li = 1, lmax
              ...
              CALL dgemv('t', n(li+1), n(li), field, dzr(1,1,li), nmax, y(  0  + nsum(li+1) ), 1, 0.0d0, dpr, 1 )
              CALL dgemv('t', n(li+1), n(li), field, dzr(1,1,li), nmax, y(ntot + nsum(li+1) ), 1, 0.0d0, dpi, 1 )
           ...
           vt(kt,1) = 2.0_dpk * vt(kt,1) !+ A_t( time(kt) ) !! + 2.0D+00*A_t(time(ikt))
           vt(kt,2) = 2.0_dpk * vt(kt,2) !+ A_t( time(kt) ) !! + 2.0D+00*A_t(time(ikt))
           vt(kt,3) = 2.0_dpk * vt(kt,3) !+ A_t( time(kt) ) !! + 2.0D+00*A_t(time(ikt))
           vt(kt,4) = vt(kt,1) + vt(kt,2) + vt(kt,3) !total = sum of partial
...
!  don't uncomment            at(kt) =   & at(kt)
!  don't uncomment                       & + DOT_PRODUCT( y(nsum(li):nsum(li) + n(li) - 1 ), dpr)           &
!  don't uncomment                       & + DOT_PRODUCT( y(ntot+nsum(li):ntot+nsum(li) + n(li) - 1 ), dpi)
...
!                WRITE(ndip,'(2x,5e25.12)') time(ikt), at(ikt), vt(ikt),rt(ikt), E_t(time(ikt))
                WRITE(ndip,'(2x,8e25.12)') time(ikt),         & !rt(ikt), &                  !r(t)
                     &    vt(ikt,1),vt(ikt,2),vt(ikt,3),vt(ikt,4), &                  !v(t) (in vel gauge)    1, 2, 3, Total
...
WRITE(*,*) time(ikt),         & !rt(ikt), &                  !r(t)
    ...
    ! &    at(ikt) + E_t(time(ikt)), & !a(t) (in len gauge)
     ...
     &    E_t(time(ikt))              !E(t)
       ...
       WRITE(*,*) 'Writing coe.dat after cycle and time = ', l, tim
