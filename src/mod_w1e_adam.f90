...
!
   SUBROUTINE WRITE_ALL_WF1E_ADAM(nplot, l, en, ce)
     ...
     loop_eigenstates: DO i = 1, SIZE(en)
        ...
        !save radial wavefunction P_nl(r) on [0, R] in binary
        WRITE(ifile) (p(j), j = 1, np)

        !save radial wavefunction P_nl(r) on [0, R] in ascii
     ...
     CLOSE(nout)
     CLOSE(ifile)
   END SUBROUTINE WRITE_ALL_WF1E_ADAM
   !
   !
   !
   !
   SUBROUTINE WRITE_LOPT_WF1E(nplot, l, en, ce)
	...
	DO l = 1, nl
        	DO ir = 1, np
        		WRITE(nout,'(3E20.10)') r(ir), p(ir,l)
        	ENDDO
	END DO
