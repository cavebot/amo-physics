...
!  write(*,*) "working l2 = ", l2_int
  ...
  write(*,*)
  write(*,*) "Method 7: Scaled to total energy + All states and configurations (no Ground State) + First term. Partial in L"
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       loop_configurations: do ic = 1, ic_max!sum from configuration 1 to max number of configurations, for the total  L
        ...
        !   cycle loop_configurations
         ...
         N2e:DO ie = starting_state, (ie_max)   !SUM OVER N
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
WRITE(Hout_outfile, "(a,i1,a,i1,a,i1,a)") "v12t/method7/v12t_method7_noground_", l_int, ".out"
OPEN(3, file = Hout_outfile, access = 'append')

write(3,'(i2,3E20.10)') num_cycles_int, ((hlt(l_int) + fl1l2l)/ht)
close(3)
