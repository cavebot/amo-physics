...
*     .. Executable Statements ..
      ...
      PASS = .TRUE.
      TOL = 10*X02AJF()
*     Generate a random matrix in SAVB.
      DO 40 I = 1, LDB
         DO 20 J = 1, LDB
            SAVB(I,J) = G05CAF(DUM)
   20    CONTINUE
  ...
  260                CONTINUE
*
                     CALL F06QJF(SIDE,TRANS,N,PERM,K,B,LDB)
                     CALL F06QKF(SIDE,TRANS,N,RPERM,K,D,LDB)
*
                     IF (TRANS.EQ.'T') THEN
*                       Transpose P.
                        DO 300 I = 1, M
...
*                    Now C should equal B and D. Compare them.
                     DO 460 I = 1, M
                        DO 440 J = 1, K
                           ...
                           IF (DIF1.GT.TOL) THEN
                              ...
                              WRITE (NOUT,99997) 'F06QJF', SIDE, TRANS,
     +                          N, 'PERM', K, 'B', LDB, M
                           ...
                           ELSE IF (DIF2.GT.TOL) THEN
                              ...
                              WRITE (NOUT,99997) 'F06QKF', SIDE, TRANS,
     +                          N, 'RPERM', K, 'D', LDB, M
  ...
  580 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06QJF Example Program Results',/1X)
99998 FORMAT (' Element ',A,'(',I3,',',I3,') was incorrectly computed ',
     +       'by the call:')
99997 FORMAT (' CALL ',A,'(''',A,''',''',A,''',',I3,',',A,',',I3,',',A,
     +       ',',I3,'),     with M =',I3)
99996 FORMAT ('  as ',1P,D13.5,' instead of ',D13.5,' .')
99995 FORMAT (' F06QJF Example Program ends OK')
99994 FORMAT (' F06QJF Example Program ends with ERRORS')
      END
