...
*     .. Local Scalars ..
      DOUBLE PRECISION ALPHA, DIF, DUM, TOL
...
*           Copy A from SAVA.
  ...
  140       CONTINUE
*           Factorise A into QR.
            CALL F06QQF(N,ALPHA,X,INCX,A,LDA,C,S)
...
*           Set Q = Q'.
            DO 260 I = 1, N
               S(I) = -S(I)
  260       CONTINUE
*           Insert zeros below diagonal of A for call to F06QXF.
            DO 300 I = 1, N
               DO 280 J = 1, I - 1
                  A(I,J) = ZERO
  280          CONTINUE
  300       CONTINUE
*           Premultiply R by Q.
            CALL F06QXF('L','B','B',N+1,N,1,N+1,C,S,A,LDA)
...
*                                         (ALPHA*X')
  ...
  400 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06QQF Example Program Results',/1X)
99998 FORMAT (' Element A(',I3,',',I3,') was altered by the call:')
99997 FORMAT (' CALL F06QQF(',I3,',',1P,D13.5,',X,',I3,',A,',I3,',C,S)')
...
99991 FORMAT (' F06QQF Example Program ends OK')
99990 FORMAT (' F06QQF Example Program ends with ERRORS')
      END
