...
*     .. Local Scalars ..
      ...
      LOGICAL          FATAL, LTESTT, SAME, SFATAL, TRACE, TSTERR
...
*     .. Executable Statements ..
      WRITE (NOUT,*) 'F06ZAF Example Program Results'
      READ (NIN,*)
      NOUTC = NOUT
      NTRA = NOUT
*
...
*
   ...
   60 READ (NIN,99988,END=120) SNAMET, LTESTT
      DO 80 I = 1, NSUBS
         IF (SNAMET.EQ.SNAMES(I)(8:13)) GO TO 100
   80 CONTINUE
...
*           Subprogram is not to be tested.
            WRITE (NOUT,99987) SNAMES(ISNUM) (8:13)
         ELSE
            SRNAMT = SNAMES(ISNUM)
*           Test error exits.
            IF (TSTERR) THEN
...
*
     ...
     +                         SNAME(8:13), TRANSA, TRANSB, M, N, K,
     +                         ALPHA, LDA, LDB, BETA, LDC
                           CALL ZGEMM(TRANSA,TRANSB,M,N,K,ALPHA,AA,LDA,
     +                                BB,LDB,BETA,CC,LDC)
*
...
*
  240 CONTINUE
      ...
      WRITE (NOUT,99995) NC, SNAME(8:13), TRANSA, TRANSB, M, N, K,
     +  ALPHA, LDA, LDB, BETA, LDC
*
  260 CONTINUE
      RETURN
...
*     .. External Functions ..
      ...
      EXTERNAL         LCE, LCERES
*     .. External Subroutines ..
      EXTERNAL         ZHEMM, CMAKE, CMMCH, ZSYMM
*     .. Intrinsic Functions ..
      INTRINSIC        MAX
*     .. Common blocks ..
      COMMON           /INFOC/INFOT, NOUTC, OK, LERR
...
*                       Call the subroutine.
*
                        IF (TRACE) WRITE (NTRA,99995) NC, SNAME(8:13),
     +                      SIDE, UPLO, M, N, ALPHA, LDA, LDB, BETA, LDC
                        IF (CONJ) THEN
                           CALL ZHEMM(SIDE,UPLO,M,N,ALPHA,AA,LDA,BB,LDB,
     +                                BETA,CC,LDC)
                        ELSE
                           CALL ZSYMM(SIDE,UPLO,M,N,ALPHA,AA,LDA,BB,LDB,
     +                                BETA,CC,LDC)
                        END IF
*
*                       Check if error-exit was taken incorrectly.
...
*
  220 CONTINUE
      WRITE (NOUT,99996) SNAME(8:13)
      WRITE (NOUT,99995) NC, SNAME(8:13), SIDE, UPLO, M, N, ALPHA, LDA,
     +  LDB, BETA, LDC
*
  240 CONTINUE
      RETURN
...
*     .. External Functions ..
      ...
      EXTERNAL         LCE, LCERES
*     .. External Subroutines ..
      EXTERNAL         CMAKE, CMMCH, ZTRMM, ZTRSM
*     .. Intrinsic Functions ..
      INTRINSIC        MAX
*     .. Common blocks ..
      COMMON           /INFOC/INFOT, NOUTC, OK, LERR
...
*
                           IF (SNAME(11:12).EQ.'MM') THEN
                              IF (TRACE) WRITE (NTRA,99995) NC,
     +                            SNAME(8:13), SIDE, UPLO, TRANSA, DIAG,
     +                            M, N, ALPHA, LDA, LDB
...
*
  300 CONTINUE
      WRITE (NOUT,99996) SNAME(8:13)
      WRITE (NOUT,99995) NC, SNAME(8:13), SIDE, UPLO, TRANSA, DIAG, M,
     +  N, ALPHA, LDA, LDB
*
  320 CONTINUE
      RETURN
...
*     .. External Functions ..
      ...
      EXTERNAL         LCE, LCERES
*     .. External Subroutines ..
      EXTERNAL         ZHERK, CMAKE, CMMCH, ZSYRK
*     .. Intrinsic Functions ..
...
*
     ...
     +                         SNAME(8:13), UPLO, TRANS, N, K, RALPHA,
     +                         LDA, RBETA, LDC
                           CALL ZHERK(UPLO,TRANS,N,K,RALPHA,AA,LDA,
     +                                RBETA,CC,LDC)
                        ELSE
                           IF (TRACE) WRITE (NTRA,99993) NC,
     +                         SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +                         LDA, BETA, LDC
                           CALL ZSYRK(UPLO,TRANS,N,K,ALPHA,AA,LDA,BETA,
     +                                CC,LDC)
                        END IF
*
*                       Check if error-exit was taken incorrectly.
...
*
  240 CONTINUE
      WRITE (NOUT,99996) SNAME(8:13)
      IF (CONJ) THEN
         WRITE (NOUT,99994) NC, SNAME(8:13), UPLO, TRANS, N, K, RALPHA,
     +     LDA, RBETA, LDC
      ELSE
         WRITE (NOUT,99993) NC, SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +     LDA, BETA, LDC
      END IF
*
  260 CONTINUE
...
*     .. External Functions ..
      ...
      EXTERNAL         LCE, LCERES
*     .. External Subroutines ..
      EXTERNAL         ZHER2K, CMAKE, CMMCH, ZSYR2K
*     .. Intrinsic Functions ..
...
*
     ...
     +                         SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +                         LDA, LDB, RBETA, LDC
                           CALL ZHER2K(UPLO,TRANS,N,K,ALPHA,AA,LDA,BB,
     +                                 LDB,RBETA,CC,LDC)
                        ELSE
                           IF (TRACE) WRITE (NTRA,99993) NC,
     +                         SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +                         LDA, LDB, BETA, LDC
                           CALL ZSYR2K(UPLO,TRANS,N,K,ALPHA,AA,LDA,BB,
     +                                 LDB,BETA,CC,LDC)
                        END IF
*
*                       Check if error-exit was taken incorrectly.
...
*
  300 CONTINUE
      WRITE (NOUT,99996) SNAME(8:13)
      IF (CONJ) THEN
         WRITE (NOUT,99994) NC, SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +     LDA, LDB, RBETA, LDC
      ELSE
         WRITE (NOUT,99993) NC, SNAME(8:13), UPLO, TRANS, N, K, ALPHA,
     +     LDA, LDB, BETA, LDC
