...
*     Skip heading in data file
      ...
      IF (N.LE.NMAX .AND. M.LT.MMAX) THEN
         IF (WEIGHT.EQ.'W' .OR. WEIGHT.EQ.'w') THEN
            DO 20 I = 1, N
               READ (NIN,*) (X(I,J),J=1,M), Y(I), T(I), WT(I)
   20       CONTINUE
         ELSE
            DO 40 I = 1, N
               READ (NIN,*) (X(I,J),J=1,M), Y(I), T(I)
   40       CONTINUE
         END IF
...
*        Set control parameters
         ...
         IFAIL = -1
*
         CALL G02GBF(LINK,MEAN,OFFSET,WEIGHT,N,X,NMAX,M,ISX,IP,Y,T,WT,
...
*
   ...
   80       CONTINUE
     ...
     +        '       N          Y        FV      Residual      H'
            WRITE (NOUT,*)
            DO 100 I = 1, N
               WRITE (NOUT,99996) T(I), Y(I), V(I,2), V(I,5), V(I,6)
  100       CONTINUE
         END IF
      END IF
      STOP
