...
*        -- Print
   ...
   20 CONTINUE
      STOP
*
99999 FORMAT (' F06GBF Example Program Results',/1X)
99998 FORMAT ('                                    ----- PASS -----')
      END
      SUBROUTINE HEADER
*     .. Parameters ..
...
*     .. Local Arrays ..
     ...
     +                  MWPCS(5), MWPCT(5)
      ...
      INTEGER           ITRUE3(5)
*     .. External Functions ..
      DOUBLE PRECISION  F06JJF, F06JKF
      INTEGER           F06JMF
      EXTERNAL          F06JJF, F06JKF, F06JMF
*     .. External Subroutines ..
      EXTERNAL          CTEST, F06GDF, F06JDF, ITEST1, STEST1
*     .. Intrinsic Functions ..
      INTRINSIC         MAX
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*           .. Set vector arguments ..
   ...
   20       CONTINUE
            IF (ICASE.EQ.6) THEN
*              .. SCNRM2 -- F06JJF ..
               CALL STEST1(F06JJF(N,CX,INCX),STRUE2(NP1),STRUE2(NP1),
     +                     SFAC)
            ELSE IF (ICASE.EQ.7) THEN
*              .. SCASUM -- F06JKF ..
               CALL STEST1(F06JKF(N,CX,INCX),STRUE4(NP1),STRUE4(NP1),
     +                     SFAC)
            ELSE IF (ICASE.EQ.8) THEN
*              .. CSCAL -- F06GDF ..
               CALL F06GDF(N,CA,CX,INCX)
               CALL CTEST(LEN,CX,CTRUE5(1,NP1,INCX),CTRUE5(1,NP1,INCX),
     +                    SFAC)
            ELSE IF (ICASE.EQ.9) THEN
*              .. CSSCAL -- F06JDF ..
               CALL F06JDF(N,SA,CX,INCX)
               CALL CTEST(LEN,CX,CTRUE6(1,NP1,INCX),CTRUE6(1,NP1,INCX),
     +                    SFAC)
            ELSE IF (ICASE.EQ.10) THEN
*              .. ICAMAX -- F06JMF ..
               CALL ITEST1(F06JMF(N,CX,INCX),ITRUE3(NP1))
            ELSE
...
*     .. External Functions ..
      COMPLEX*16        F06GAF, F06GBF
      EXTERNAL          F06GAF, F06GBF
*     .. External Subroutines ..
      EXTERNAL          CTEST, F06GCF, F06GFF, F06GGF
*     .. Intrinsic Functions ..
      INTRINSIC         ABS, MIN
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*           .. initialize all argument arrays ..
   ...
   20       CONTINUE
            IF (ICASE.EQ.1) THEN
*              .. CDOTC -- F06GBF ..
               CDOT(1) = F06GBF(N,CX,INCX,CY,INCY)
               CALL CTEST(1,CDOT,CT6(KN,KI),CSIZE1(KN),SFAC)
            ELSE IF (ICASE.EQ.2) THEN
*              .. CDOTU -- F06GAF ..
               CDOT(1) = F06GAF(N,CX,INCX,CY,INCY)
               CALL CTEST(1,CDOT,CT7(KN,KI),CSIZE1(KN),SFAC)
            ELSE IF (ICASE.EQ.3) THEN
*              .. CAXPY -- F06GCF ..
               CALL F06GCF(N,CA,CX,INCX,CY,INCY)
               CALL CTEST(LENY,CY,CT8(1,KN,KI),CSIZE2(1,KSIZE),SFAC)
            ELSE IF (ICASE.EQ.4) THEN
*              .. CCOPY -- F06GFF ..
               CALL F06GFF(N,CX,INCX,CY,INCY)
               CALL CTEST(LENY,CY,CT10Y(1,KN,KI),CSIZE3,1.0D0)
            ELSE IF (ICASE.EQ.5) THEN
*              .. CSWAP -- F06GGF ..
               CALL F06GGF(N,CX,INCX,CY,INCY)
