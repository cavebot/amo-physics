...
*     .. Local Arrays ..
      DOUBLE PRECISION A(LDA,LDA), B(LDA,LDA), C(LDA), S(LDA),
     +                 SAVA(LDA,LDA)
      INTEGER          ENS(NMAX)
*     .. External Functions ..
...
*                          Make a copy of A in SAVA.
   ...
   80                      CONTINUE
*                          Do the computation.
                           CALL F06QMF(UPLO,PIVOT,DIRECT,N,K1,K2,C,S,A,
     +                                 LDA)
                           CALL F06QXF('Right',PIVOT,DIRECT,N,N,K1,K2,C,
     +                                 S,B,LDA)
                           CALL F06QXF('Left',PIVOT,DIRECT,N,N,K1,K2,C,
     +                                 S,B,LDA)
...
*                          computed correctly.
                           DO 160 I = MAX(K1,1), K2
                              DO 140 J = I, K2
                                 IF (UPLO.EQ.'L') THEN
                                    IF (ABS(A(J,I)-B(J,I)).GT.TOL) THEN
  ...
  320 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06QMF Example Program Results',/1X)
...
99996 FORMAT (' Element A(',I3,',',I3,') was computed as ',1P,D13.5,
     +       ' by the call:')
99995 FORMAT (' CALL F06QMF(''',A,''',''',A,''',''',A,''',',I3,',',I3,
     +       ',',I3,',C,S,A,',I3,')')
99994 FORMAT (' and as ',1P,D13.5,' by the calls:')
99993 FORMAT (' CALL F06QXF(''',A,''',''',A,''',''',A,''',',I3,',',I3,
     +       ',',I3,',',I3,',C,S,A,',I3,')')
99992 FORMAT (' These results should be identical.')
99991 FORMAT (' F06QMF Example Program ends OK')
99990 FORMAT (' F06QMF Example Program ends with ERRORS')
      END
