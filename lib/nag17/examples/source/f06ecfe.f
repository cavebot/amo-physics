...
*        -- Print
   ...
   20 CONTINUE
      STOP
*
99999 FORMAT (' F06ECF Example Program Results',/1X)
99998 FORMAT ('                                    ----- PASS -----')
      END
      SUBROUTINE HEADER
*     .. Parameters ..
...
*     .. Local Arrays ..
     ...
     +                  SA(10), STEMP(1), STRUE(8), SX(8)
      INTEGER           ITRUE2(5)
*     .. External Functions ..
      DOUBLE PRECISION  F06EJF, F06EKF
      INTEGER           F06JLF
      EXTERNAL          F06EJF, F06EKF, F06JLF
*     .. External Subroutines ..
      EXTERNAL          F06EDF, ITEST1, STEST, STEST1
*     .. Intrinsic Functions ..
      INTRINSIC         MAX
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*              .. SNRM2 -- F06EJF ..
               STEMP(1) = DTRUE1(NP1)
               CALL STEST1(F06EJF(N,SX,INCX),STEMP(1),STEMP(1),SFAC)
            ELSE IF (ICASE.EQ.8) THEN
*              .. SASUM -- F06EKF ..
               STEMP(1) = DTRUE3(NP1)
               CALL STEST1(F06EKF(N,SX,INCX),STEMP(1),STEMP(1),SFAC)
            ELSE IF (ICASE.EQ.9) THEN
*              .. SSCAL -- F06EDF ..
               CALL F06EDF(N,SA((INCX-1)*5+NP1),SX,INCX)
               DO 40 I = 1, LEN
...
*              .. ISAMAX -- F06JLF ..
               CALL ITEST1(F06JLF(N,SX,INCX),ITRUE2(NP1))
            ELSE
...
*     .. External Functions ..
      ...
      EXTERNAL          F06EAF
*     .. External Subroutines ..
      EXTERNAL          F06ECF, F06EFF, F06EGF, STEST, STEST1
*     .. Intrinsic Functions ..
      INTRINSIC         ABS, MIN
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*
            IF (ICASE.EQ.1) THEN
*              .. SDOT -- F06EAF ..
               CALL STEST1(F06EAF(N,SX,INCX,SY,INCY),DT7(KN,KI),
     +                     SSIZE1(KN),SFAC)
            ELSE IF (ICASE.EQ.2) THEN
*              .. SAXPY -- F06ECF ..
               CALL F06ECF(N,SA,SX,INCX,SY,INCY)
               DO 40 J = 1, LENY
                  STY(J) = DT8(J,KN,KI)
   40          CONTINUE
...
*              .. SCOPY -- F06EFF ..
               DO 60 I = 1, 7
                  STY(I) = DT10Y(I,KN,KI)
   60          CONTINUE
               CALL F06EFF(N,SX,INCX,SY,INCY)
               CALL STEST(LENY,SY,STY,SSIZE2(1,1),1.0D0)
            ELSE IF (ICASE.EQ.6) THEN
*              .. SSWAP -- F06EGF ..
               CALL F06EGF(N,SX,INCX,SY,INCY)
               DO 80 I = 1, 7
                  STX(I) = DT10X(I,KN,KI)
                  STY(I) = DT10Y(I,KN,KI)
   80          CONTINUE
...
*              .. SROT -- F06EPF ..
               DO 20 I = 1, 7
                  ...
                  STY(I) = DT9Y(I,KN,KI)
   20          CONTINUE
               CALL F06EPF(N,SX,INCX,SY,INCY,SC,SS)
  ...
  160 CONTINUE
      ...
      DO 200 I = 1, 11
         ...
         DO 180 K = 1, 5
            ...
            MWPSTY(K) = MWPTY(I,K)
  180    CONTINUE
         CALL F06EPF(MWPN(I),COPYX,INCX,COPYY,INCY,MWPC(I),MWPS(I))
