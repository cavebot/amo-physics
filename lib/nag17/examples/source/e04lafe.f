...
*     .. Scalar Arguments ..
      INTEGER          LH, N
*     .. Array Arguments ..
      DOUBLE PRECISION HESDC(N), HESLC(LH), XC(N)
*     .. Local Scalars ..
      DOUBLE PRECISION X1, X2, X3, X4
*     .. Executable Statements ..
      X1 = XC(1)
      ...
      X3 = XC(3)
      X4 = XC(4)
      HESDC(1) = 2.0D0 + 120.0D0*(X1-X4)**2
      HESDC(2) = 200.0D0 + 12.0D0*(X2-2.0D0*X3)**2
      HESDC(3) = 10.0D0 + 48.0D0*(X2-2.0D0*X3)**2
      HESDC(4) = 10.0D0 + 120.0D0*(X1-X4)**2
      HESLC(1) = 20.0D0
      HESLC(2) = 0.0D0
      HESLC(3) = -24.0D0*(X2-2.0D0*X3)**2
      HESLC(4) = -120.0D0*(X1-X4)**2
      HESLC(5) = 0.0D0
      HESLC(6) = -10.0D0
      RETURN
      END
