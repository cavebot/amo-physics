...
*     .. Parameters ..
      ...
      COMPLEX*16       CZERO
      PARAMETER        (RROGUE=-1.0D+10,CZERO=(0.0D+0,0.0D+0))
      INTEGER          LDA, NMAX
...
*     .. Local Scalars ..
      ...
      CHARACTER*1      SIDE
*     .. Local Arrays ..
      COMPLEX*16       A(LDA,LDA), S(LDA), SAVA(LDA,LDA), SAVS(LDA)
...
*     .. Executable Statements ..
   ...
   40                   CONTINUE
   60                CONTINUE
                     IF (N.GT.0) S(N) = CROGUE
                     CALL F06TSF(SIDE,N,K1,K2,C,S,A,LDA)
                     ...
                     DO 100 I = 2, N
                        DO 80 J = 1, I - 1
                           IF (A(I,J).NE.CROGUE) THEN
                              ...
                              WRITE (NOUT,99993) 'TSF', SIDE, N, K1, K2,
     +                          LDA
                              WRITE (NOUT,99996)
                              PASS = .FALSE.
                              GO TO 460
...
*                          Postmultiply A by D.
  ...
  200                      CONTINUE
                        END IF
                     END IF
                     CALL F06TWF(SIDE,N,K1,K2,C,S,A,LDA)
*                    Check whether A has been changed below diagonal by
*                    F06TWF.
                     DO 240 I = 2, N
                        DO 220 J = 1, I - 1
                           IF (A(I,J).NE.CROGUE) THEN
                              WRITE (NOUT,99998) I, J
                              WRITE (NOUT,99993) 'TWF', SIDE, N, K1, K2,
     +                          LDA
                              WRITE (NOUT,99996)
                              PASS = .FALSE.
                              GO TO 460
...
*                    Compare with the saved arrays.
  ...
  460 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06TSF Example Program Results',/1X)
...
99992 FORMAT ('  as (',1P,D13.5,',',D13.5,') instead of (',D13.5,',',
     +       D13.5,').')
99991 FORMAT (' F06TSF Example Program ends OK')
99990 FORMAT (' F06TSF Example Program ends with ERRORS')
      END
