...
*     .. Local Scalars ..
      ...
      LOGICAL          PASS
*     .. Local Arrays ..
      COMPLEX*16       A(LDA,LDA), B(LDA,LDA), C(LDA,LDA)
      INTEGER          ENS(MAXN)
*     .. External Functions ..
      DOUBLE PRECISION G05CAF, X02AJF
      EXTERNAL         G05CAF, X02AJF
*     .. External Subroutines ..
      EXTERNAL         CPYCHK, F06TFF, F06THF
*     .. Intrinsic Functions ..
...
*     .. Executable Statements ..
   ...
   20          CONTINUE
   40       CONTINUE
            CONST = DCMPLX(G05CAF(DUM),G05CAF(DUM))
            DIAG = DCMPLX(G05CAF(DUM),G05CAF(DUM))
            CALL F06THF('L',M,N,CONST,DIAG,A,LDA)
            CALL F06TFF('L',M,N,A,LDA,C,LDA)
            CALL CPYCHK(M,N,A,C,LDA,PASS,NOUT,'L','A','C')
            IF ( .NOT. PASS) GO TO 180
            CALL F06THF('G',M,N,CONST,DIAG,B,LDA)
            CALL F06TFF('G',M,N,B,LDA,C,LDA)
            CALL CPYCHK(M,N,B,C,LDA,PASS,NOUT,'G','B','C')
            IF ( .NOT. PASS) GO TO 180
            CALL F06THF('U',M,N,ZERO,DIAG,B,LDA)
            CALL F06TFF('U',M,N,B,LDA,C,LDA)
...
*           Now lower triangles of A and B should be equal.
   ...
   60          CONTINUE
   80       CONTINUE
            CALL F06THF('G',M,N,CONST,DIAG,A,LDA)
            CALL F06THF('L',M,N,CONST,DIAG,B,LDA)
            CALL F06THF('U',M,N,CONST,DIAG,B,LDA)
            DO 120 I = 1, M
               DO 100 J = 1, N
                  IF (ABS(A(I,J)-B(I,J)).GT.TOL) THEN
                     WRITE (NOUT,99998)
  ...
  180 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06THF Example Program Results',/1X)
99998 FORMAT (' The sequence:')
99997 FORMAT (' CALL F06THF(''',A,''',',I3,',',I3,', (',1P,D12.5,',',
     +       D12.5,'),',/'                          (',D12.5,',',D12.5,
     +       '),',A,',',I3,')')
99996 FORMAT (' returned elements A(I,J) and B(I,J), for I =',I3,' and',
     +       ' J =',I3,',',/'  as (',1P,D12.5,',',D12.5,') and (',D12.5,
     +       ',',D12.5,') respectively.',/'  These elements should be ',
     +       'equal.')
99995 FORMAT (' F06THF Example Program ends OK')
99994 FORMAT (' F06THF Example Program ends with ERRORS')
      END
*
...
*     .. Scalar Arguments ..
      ...
      LOGICAL           PASS
      CHARACTER         MAT, MAT1, MAT2
*     .. Array Arguments ..
      COMPLEX*16        A(LDA,LDA), B(LDA,LDA)
*     .. Local Scalars ..
      INTEGER           I, J
*     .. Executable Statements ..
      DO 40 I = 1, M
...
99998 FORMAT (' CALL F06TFF(''',A,''',',I3,',',I3,',',A,',',I3,',',A,
     +       ',',I3,')')
99997 FORMAT (' returned ',A,'(',I3,',',I3,') as (',1P,D12.5,',',D12.5,
     +       ')',/' instead of (',D12.5,',',D12.5,') .')
      END
