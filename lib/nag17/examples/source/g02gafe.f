...
*        Set control parameters
         ...
         IFAIL = -1
*
         CALL G02GAF(LINK,MEAN,OFFSET,WEIGHT,N,X,NMAX,M,ISX,IP,Y,WT,S,A,
     +               RSS,IDF,B,IRANK,SE,COV,V,NMAX,TOL,MAXIT,IPRINT,EPS,
     +               WK,IFAIL)
*
         IF (IFAIL.EQ.0 .OR. IFAIL.GE.6) THEN
            WRITE (NOUT,*)
            WRITE (NOUT,99999) 'Residual sum of squares = ', RSS,
     +        ' Degrees of freedom = ', IDF
            WRITE (NOUT,*)
            WRITE (NOUT,*) '      Estimate    Standard error'
            WRITE (NOUT,*)
            DO 80 I = 1, IP
               WRITE (NOUT,99998) B(I), SE(I)
   80       CONTINUE
            WRITE (NOUT,*)
            WRITE (NOUT,*) '     Y        FV     Residual       H'
            WRITE (NOUT,*)
            DO 100 I = 1, N
               WRITE (NOUT,99997) Y(I), V(I,2), V(I,5), V(I,6)
  100       CONTINUE
         END IF
      END IF
      STOP
*
99999 FORMAT (1X,A,D12.4,A,I2)
99998 FORMAT (1X,2F14.4)
99997 FORMAT (1X,F7.1,F10.2,F12.4,F10.3)
      END
