...
*     .. Local Scalars ..
      COMPLEX*16       ALPHA, CROGUE, DIAG, T
      DOUBLE PRECISION DIF, DUM, TOL
      INTEGER          I, INC1, INC2, INCX, INCY, J, N, NN
      LOGICAL          PASS
*     .. Local Arrays ..
      COMPLEX*16       A(LDA,LDA), S1(LDA), S2(LDA), SAVA(LDA,LDA),
     +                 SAVX(LDA*INCMAX), X(LDA*INCMAX), Y(LDA*INCMAX)
      DOUBLE PRECISION C1(LDA), C2(LDA)
      INTEGER          ENS(NMAX), INCS(NINCS)
*     .. External Functions ..
      DOUBLE PRECISION G05CAF, X02AJF
      EXTERNAL         G05CAF, X02AJF
*     .. External Subroutines ..
      EXTERNAL         F06CCF, F06TPF, F06TXF
*     .. Intrinsic Functions ..
   ...
   40    CONTINUE
         DO 460 INC1 = 1, NINCS
            ...
            DO 80 I = 1, (N-1)*INCX + 1, INCX
               SAVX(I) = DCMPLX(G05CAF(DUM),G05CAF(DUM))
   ...
   80       CONTINUE
            DO 440 INC2 = 1, NINCS
               ...
               DO 120 I = 1, (N-1)*INCY + 1, INCY
                  Y(I) = DCMPLX(G05CAF(DUM),G05CAF(DUM))
...
*              Factorise (ALPHA*X*Y' + A) into QR.
               CALL F06TPF(N,ALPHA,X,INCX,Y,INCY,A,LDA,C2,S2)
*              DIAG is the unitary diagonal matrix.
               IF (N.GT.1) THEN
                  DIAG = DCONJG(S2(N))
               END IF
*              Regenerate C1 and S1 from X.
               DO 200 I = 1, N - 1
                  CALL F06CCF(X((I-1)*INCX+1),C1(I),S1(I))
  200          CONTINUE
...
*              Insert zeros below diagonal of A for call to F06TXF.
  ...
  360             CONTINUE
  380          CONTINUE
*              Premultiply R by the diagonal matrix D.
               IF (N.GT.1) A(N,N) = DIAG*A(N,N)
*              Premultiply R by the rest of Q.
               CALL F06TXF('L','B','B',N,N,1,N,C2,S2,A,LDA)
               CALL F06TXF('L','B','F',N,N,1,N,C1,S1,A,LDA)
*              Now (A) should be the same as SAVA + ALPHA*SAVX*Y'.
  ...
  500 IF (PASS) THEN
      ...
      STOP
*
99999 FORMAT (' F06TPF Example Program Results',/1X)
99998 FORMAT (' Element A(',I3,',',I3,') was altered by the call:')
99997 FORMAT (' CALL F06TPF(',I3,', (',1P,D13.5,',',D13.5,') ,X,',I3,
     +       ',Y,',I3,',A,',I3,',C,S)')
...
99992 FORMAT ('  as (',1P,D13.5,',',D13.5,') instead of (',D13.5,',',
     +       D13.5,').')
99991 FORMAT (' F06TPF Example Program ends OK')
99990 FORMAT (' F06TPF Example Program ends with ERRORS')
      END
