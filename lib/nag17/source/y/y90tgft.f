      DOUBLE PRECISION FUNCTION Y90TGF(NORM,MATRIX,M,N,A,LDA)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C
C  This routine originally called F06QGF.
C
C  Purpose
C  =======
C
C  Y90TGF  returns the value of the one norm,  or the Frobenius norm, or
...
C  ===========
C
C  Y90TGF returns the value
C
C     Y90TGF = ( max( abs( a( i, j ) ) ) , NORM = 'M' or 'm'
C              (
...
C
C  The type of matrix for which  Y90TGF is returned is determined by the
C  parameter MATRIX.
C
...
C  NORM  -  CHARACTER*1.
C
C           On entry,  NORM specifies the value to be returned in Y90TGF
...
C           then  M must be equal to  N. When  M = 0  then Y90TGF is set
...
C           N  must be at least zero. When  N = 0  then Y90TGF is set to
...
C
  ...
  220          CONTINUE
               VALUE = MAX(VALUE,SUM)
  240       CONTINUE
         ELSE IF ((MATRIX.EQ.'S') .OR. (MATRIX.EQ.'s')
     *            .OR. (MATRIX.EQ.'H') .OR. (MATRIX.EQ.'h')) THEN
            DO 300 J = 1, N
               SUM = ZERO
               DO 260 I = 1, J
  ...
  280          CONTINUE
               VALUE = MAX(VALUE,SUM)
  300       CONTINUE
         ELSE IF ((MATRIX.EQ.'Y') .OR. (MATRIX.EQ.'y')
     *            .OR. (MATRIX.EQ.'E') .OR. (MATRIX.EQ.'e')) THEN
            DO 360 J = 1, N
               SUM = ZERO
               DO 320 I = 1, J - 1
